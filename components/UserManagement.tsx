                            ...
                            {userList.map((user: User) => (
                                <tr key={user.username} className="bg-zinc-800 border-b border-zinc-700 hover:bg-zinc-600/50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${user.role === UserRole.ADMIN ? 'bg-green-900 text-green-300' : user.role === UserRole.MONITOR_IMPO ? 'bg-purple-900 text-purple-300' : 'bg-blue-900 text-blue-300'}`}>
                                            {user.role}
                                        </span>
                                    </td>
